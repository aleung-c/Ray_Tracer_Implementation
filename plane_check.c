				...
				t_vector3 vec_direction)
{
	...
	+ (obj->plane_obj->normale.y * origine.y) 
	+ obj->plane_obj->point.z)
	/ ((obj->plane_obj->normale.x * vec_direction.x)
	+ (obj->plane_obj->normale.z * vec_direction.z)
	+ -(obj->plane_obj->normale.y * vec_direction.y)));
	...
	if (algo.t >= 0.0)
	{
		...
		algo.tpoint.x = origine.x + algo.t * vec_direction.x;
		algo.tpoint.y = origine.y + algo.t * vec_direction.y;
		algo.tpoint.z = origine.z + algo.t * vec_direction.z;
